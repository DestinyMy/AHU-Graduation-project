...
class Individual(object):
    def __init__(self, structure=None, accuracy=0, size=0):
...
def genetic(individuals, num_classes, num_epochs, num_individuals, generations, batch_size, learning_rate):
    for i in range(generations):
        for j in range(len(individuals)):
            if individuals[j].accuracy < 1: # 该个体之前训练过就不训练
                evaluate(individuals[j], num_classes, num_epochs, batch_size, learning_rate)
